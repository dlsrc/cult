...
-- Cult Project Schema for MariaDB Server only.
-- Tested on MariaDB 10.6 and MariaDB 11.2 servers.

-- Over time, MariaDB and MySQL servers have lost much of their compatibility,
-- so the MariaDB schema does not work on MySQL servers.
-- This schema is not suitable for deployment on MySQL servers.

CREATE TABLE IF NOT EXISTS language (
  lang_id tinyint(3) unsigned NOT NULL AUTO_INCREMENT,
  name char(2) NOT NULL,
  title_path char(4) NOT NULL DEFAULT '$.ru',
  title varchar(63) NOT NULL DEFAULT '',
  status tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (status < 8),
...
CREATE OR REPLACE DEFINER = CURRENT_USER TRIGGER json_title_path
BEFORE INSERT ON language FOR EACH ROW
SET NEW.title_path = CONCAT('$.', NEW.name);
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Конфигурация компонента';

CREATE TABLE IF NOT EXISTS role (
  role_id smallint(5) unsigned NOT NULL AUTO_INCREMENT,
  code char(3) NOT NULL COMMENT 'Трехбуквенный код роли',
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  status tinyint(2) unsigned NOT NULL DEFAULT 0 CHECK (status < 16),
  nocache tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (nocache < 2),
...
CREATE TABLE IF NOT EXISTS component (
  component_id smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор компонента',
  config_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор класса конфигурации',
  role_id smallint(5) unsigned NOT NULL DEFAULT 0 COMMENT 'Идентификатор роли поумолчанию назначаемой при имплементации компонента',
  class varchar(127) NOT NULL COMMENT 'Класс компонента',
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  cache tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (cache < 2),
  singleton tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (singleton < 2),
  ...
  FOREIGN KEY component_config_fkey (config_id) REFERENCES config (config_id) ON DELETE CASCADE,
  FOREIGN KEY component_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Компонент';

CREATE TABLE IF NOT EXISTS event (
  event_id int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор события или группы событий с требованием доступа',
  component_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор компонента',
  role_id smallint(5) unsigned NOT NULL DEFAULT 0 COMMENT 'Идентификатор роли по умолчанию назначаемой при имплементации компонента',
  name varchar(127) NOT NULL DEFAULT '' COMMENT 'Имя события',
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  cache tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (cache < 2),
  access tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (access < 2),
  nav tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (nav < 2),
  PRIMARY KEY event_pkey (event_id),
  KEY event_component (component_id),
  KEY event_role (role_id),
  FOREIGN KEY event_component_fkey (component_id) REFERENCES component (component_id) ON DELETE CASCADE,
  FOREIGN KEY event_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS project (
  project_id smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор проект (шаблона) приложения',
  config_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор класса конфигурации',
  ...
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  singleton tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'Признак шаблона одиночки' CHECK (singleton < 2),
  open tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'Открыт для создания приложений' CHECK (open < 2),
  ...
  FOREIGN KEY project_config_fkey (config_id) REFERENCES config (config_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Проект (шаблон) приложения';

CREATE TABLE IF NOT EXISTS decision (
  decision_id smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Индекс паттерна локации, позиция модуля в шаблоне',
  project_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор проекта (шаблона) приложения',
  component_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор шаблона локации, индекс компонента',
  ...
  FOREIGN KEY decision_project_fkey (project_id) REFERENCES project (project_id) ON DELETE CASCADE,
  FOREIGN KEY decision_component_fkey (component_id) REFERENCES component (component_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS projectmod (
  ...
  FOREIGN KEY projectmod_project_fkey (project_id) REFERENCES project (project_id) ON DELETE CASCADE,
  FOREIGN KEY projectmod_decision_fkey (decision_id) REFERENCES decision (decision_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS build (
  build_id smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор приложения',
  project_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор проекта (шаблона) приложения',
  ...
  FOREIGN KEY build_project_fkey (project_id) REFERENCES project (project_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Приложение';

CREATE TABLE IF NOT EXISTS locate (
  locate_id smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор локации',
  decision_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор проектного решения',
  build_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор приложения',
  name varchar(31) NOT NULL DEFAULT '',
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  serial smallint(5) unsigned NOT NULL DEFAULT 1 COMMENT 'Статус локации',
  cache tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (cache < 2),
  ...
  FOREIGN KEY locate_decision_fkey (decision_id) REFERENCES decision (decision_id) ON DELETE CASCADE,
  FOREIGN KEY locate_build_fkey (build_id) REFERENCES build (build_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS config_slice (
  slice_id smallint(5) unsigned NOT NULL AUTO_INCREMENT,
  slice varchar(63) NOT NULL DEFAULT '',
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  serial smallint(5) unsigned NOT NULL DEFAULT 1000,
...
CREATE TABLE IF NOT EXISTS config_option (
  ...
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  `default` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(`default`)),
  mlang tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (mlang < 2),
  serial smallint(5) unsigned NOT NULL DEFAULT 1000,
  global tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (global < 2),
  active tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (active < 2),
  ...
  FOREIGN KEY option_config_fkey (config_id) REFERENCES config (config_id) ON DELETE CASCADE,
  FOREIGN KEY option_slice_fkey (slice_id) REFERENCES config_slice (slice_id) ON DELETE CASCADE,
  FOREIGN KEY option_type_fkey (type_id) REFERENCES config_type (type_id) ON DELETE CASCADE,
  FOREIGN KEY option_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS config_enum (
  ...
  valuebykey tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (valuebykey < 2),
  ...
  FOREIGN KEY config_enum_option_fkey (option_id) REFERENCES config_option (option_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Перечисляемые значения опций';

CREATE TABLE IF NOT EXISTS config_data (
  option_id smallint(5) unsigned NOT NULL,
  locate_id smallint(5) unsigned NOT NULL,
  value longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(value)),
  mlang tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (mlang < 2),
  local tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (local < 2),
  PRIMARY KEY data_pkey (option_id, locate_id),
  KEY data_locate_id (locate_id),
  KEY data_locate (local),
  FOREIGN KEY data_option_fkey (option_id) REFERENCES config_option (option_id) ON DELETE CASCADE,
  FOREIGN KEY data_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS plugin (
  ...
  FOREIGN KEY plugin_config_fkey (config_id) REFERENCES config (config_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS access (
  ...
  FOREIGN KEY access_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY access_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS addon (
  ...
  serial smallint(5) unsigned NOT NULL DEFAULT 1,
  active tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (active < 2),
  PRIMARY KEY addon_pkey (locate_id, plugin_id),
  KEY addon_plugin (plugin_id),
  FOREIGN KEY addon_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY addon_plugin_fkey (plugin_id) REFERENCES plugin (plugin_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Плагины подключенные к локациям';

CREATE TABLE IF NOT EXISTS attach (
  plugin_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор плагина',
  role_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор роли',
  PRIMARY KEY attach_pkey (plugin_id, role_id),
  KEY attach_role (role_id),
  FOREIGN KEY attach_plugin_fkey (plugin_id) REFERENCES plugin (plugin_id) ON DELETE CASCADE,
  FOREIGN KEY attach_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS cache (
  ...
  FOREIGN KEY cache_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY cache_event_fkey (event_id) REFERENCES event (event_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS cheat (
  cheat_id smallint(5) unsigned NOT NULL AUTO_INCREMENT,
  name varchar(31) NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS control (
  ...
  config_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор класса конфигурации',
  event_id int(10) unsigned NOT NULL COMMENT 'Идентификатор активного события элемента управления',
  class varchar(63) NOT NULL,
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  self tinyint(1) NOT NULL DEFAULT 0 CHECK (self < 2),
  ...
  FOREIGN KEY control_config_fkey (config_id) REFERENCES config (config_id) ON DELETE CASCADE,
  FOREIGN KEY control_event_fkey (event_id) REFERENCES event (event_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS view (
  ...
  FOREIGN KEY view_control_fkey (control_id) REFERENCES control (control_id) ON DELETE CASCADE,
  FOREIGN KEY view_target_fkey (target) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY view_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS delegate (
  delegate_id smallint(5) unsigned NOT NULL AUTO_INCREMENT,
  class varchar(63) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `group` (
  group_id smallint(5) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор группы',
  code char(3) NOT NULL COMMENT 'Трехбуквенный код группы',
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  users_limit tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT 'Предел пользователей группы, 0 без ограничений',
  need_email tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'В группу можно включать пользователя с подтвержденным адресом электронной почты' CHECK (need_email < 2),
  need_phone tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'В группу можно включать пользователя с подтвержденным номером телефона' CHECK (need_phone < 2),
  status tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'Статус, битовая маска флагов автоподключения, подключения, удаления' CHECK(status < 16),
...
CREATE TABLE IF NOT EXISTS icon (
  icon_id smallint(3) unsigned NOT NULL AUTO_INCREMENT,
  name varchar(31) NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS markup (
  markup_id smallint(5) unsigned NOT NULL AUTO_INCREMENT,
  ...
  FOREIGN KEY markup_class_fkey (class_id) REFERENCES markup_class (class_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS menu (
  ...
  markup_id smallint(5) unsigned NOT NULL,
  ...
  FOREIGN KEY menu_markup_fkey (markup_id) REFERENCES markup (markup_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Меню';

CREATE TABLE IF NOT EXISTS menu_responsive (
  menu_id tinyint(3) unsigned NOT NULL AUTO_INCREMENT,
  for_id smallint(6) unsigned NOT NULL,
  markup_id smallint(6) unsigned NOT NULL,
  source varchar(31) NOT NULL DEFAULT '',
  icon varchar(31) NOT NULL DEFAULT '',
  side tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (side < 2),
  displace tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (displace < 2),
  ...
  FOREIGN KEY menu_responsive_for_fkey (for_id) REFERENCES markup (markup_id) ON DELETE CASCADE,
  FOREIGN KEY menu_responsive_markup_fkey (markup_id) REFERENCES markup (markup_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS navigate (
  ...
  FOREIGN KEY navigate_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY navigate_menu_fkey (menu_id) REFERENCES menu (menu_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Меню для каждой локации.';

CREATE TABLE IF NOT EXISTS reference (
  reference_id smallint(5) unsigned NOT NULL AUTO_INCREMENT,
  code char(3) NOT NULL COMMENT 'Трехбуквенный код ссылки',
  locate_id smallint(5) unsigned NOT NULL,
  event_id int(10) unsigned DEFAULT NULL,
  value varchar(255) NOT NULL DEFAULT '',
  url varchar(255) NOT NULL DEFAULT '',
  text longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  active longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  title longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL	CHECK (JSON_VALID(title)),
  status tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (status < 8),
  ...
  FOREIGN KEY reference_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY reference_event_fkey (event_id) REFERENCES event (event_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS menuitem (
  ...
  menu_id tinyint(3) unsigned NOT NULL,
  submenu_id tinyint(3) unsigned DEFAULT NULL,
  reference_id smallint(5) unsigned NOT NULL,
  role_id smallint(5) unsigned DEFAULT NULL,
  serial smallint(5) unsigned NOT NULL DEFAULT 1,
  active tinyint(1) unsigned NOT NULL DEFAULT 1 CHECK (active < 2),
  icon varchar(63) NOT NULL DEFAULT '',
  empty tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (empty < 2),
  follow tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (follow < 2),
  blank tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (blank < 2),
  status tinyint(1) unsigned NOT NULL DEFAULT 0 CHECK (status < 8),
  PRIMARY KEY item_pkey (item_id),
  KEY item_menu (menu_id),
  KEY item_reference (reference_id),
  KEY item_role (role_id),
  FOREIGN KEY item_menu_fkey (menu_id) REFERENCES menu (menu_id) ON DELETE CASCADE,
  FOREIGN KEY item_submenu_fkey (submenu_id) REFERENCES menu (menu_id) ON DELETE CASCADE,
  FOREIGN KEY item_reference_fkey (reference_id) REFERENCES reference (reference_id) ON DELETE CASCADE,
  FOREIGN KEY item_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS permit (
  ...
  FOREIGN KEY permit_group_fkey (group_id) REFERENCES `group` (group_id) ON DELETE CASCADE,
  FOREIGN KEY permit_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS privilege (
  ...
  FOREIGN KEY privilege_locate_fkey (locate_id) REFERENCES locate (locate_id) ON DELETE CASCADE,
  FOREIGN KEY privilege_event_fkey (event_id) REFERENCES event (event_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS rule (
  ...
  FOREIGN KEY rule_role_fkey (role_id) REFERENCES role (role_id) ON DELETE CASCADE,
  FOREIGN KEY rule_privilege_fkey (privilege_id) REFERENCES privilege (privilege_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  parent_id int(10) unsigned NOT NULL DEFAULT 0 COMMENT 'Идентификатор связанных пользователей',
  ...
  status tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'Статус, 0 - пользователь активен' CHECK (status < 8),
...
CREATE TABLE IF NOT EXISTS team (
  ...
  group_id smallint(5) unsigned NOT NULL COMMENT 'Идентификатор группы',
  PRIMARY KEY team_pkey (user_id, group_id),
  KEY team_group (group_id),
  FOREIGN KEY team_user_fkey (user_id) REFERENCES user (user_id) ON DELETE CASCADE,
  FOREIGN KEY team_group_fkey (group_id) REFERENCES `group` (group_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS user_auth (
  ...
  KEY auth_hash (hash),
  FOREIGN KEY auth_user_fkey (user_id) REFERENCES user (user_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS user_oauth (
  ...
  user_id int(10) unsigned NOT NULL COMMENT 'Систеный идентификатор пользователя',
  ...
  verified_email tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Флаг верификации email, принимает значения 1 и -1' CHECK(verified_email <= 1 AND verified_email >= -1),
  ...
  sex tinyint(1) unsigned NOT NULL DEFAULT 0 COMMENT 'Пол пользователя (0 – не определен, 1 – женский, 2 – мужской)' CHECK(sex < 3),
  ...
  FOREIGN KEY oauth_user_fkey (user_id) REFERENCES user (user_id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS user_phone (
  ...
  KEY user_phone (phone),
  FOREIGN KEY user_phone_fkey (user_id) REFERENCES user (user_id) ON DELETE CASCADE
...
JSON_UNQUOTE(JSON_EXTRACT(locate.title, language.title_path))    AS locate,
JSON_UNQUOTE(JSON_EXTRACT(decision.title, language.title_path))  AS decision,
JSON_UNQUOTE(JSON_EXTRACT(build.title, language.title_path))     AS build,
JSON_UNQUOTE(JSON_EXTRACT(component.title, language.title_path)) AS component,
JSON_UNQUOTE(JSON_EXTRACT(plugin.title, language.title_path))    AS title,
JSON_UNQUOTE(JSON_EXTRACT(role.title, language.title_path))      AS role,
JSON_UNQUOTE(JSON_EXTRACT(`group`.title, language.title_path))   AS `group`,
...
config.class                                                   AS config,
JSON_UNQUOTE(JSON_EXTRACT(build.title, language.title_path))   AS title,
JSON_UNQUOTE(JSON_EXTRACT(project.title, language.title_path)) AS project,
...
config.class                                                 AS config,
event.name                                                   AS name,
JSON_UNQUOTE(JSON_EXTRACT(event.title, language.title_path)) AS title,
...
config.class                                                     AS config,
JSON_UNQUOTE(JSON_EXTRACT(component.title, language.title_path)) AS title,
JSON_UNQUOTE(JSON_EXTRACT(role.title, language.title_path))      AS role,
JSON_UNQUOTE(JSON_EXTRACT(config.title, language.title_path))    AS config_title,
...
CASE
  ...
  ELSE 1
END                                                              AS is_free,
decision.decision_id IS NOT NULL                                 AS is_decision,
language.lang_id                                                 AS lang_id,
language.name                                                    AS lang,
language.title                                                   AS language
FROM component
...
JSON_UNQUOTE(JSON_EXTRACT(config.title, language.title_path)) AS title,
language.lang_id                                              AS lang_id,
language.name                                                 AS lang,
language.title                                                AS language
FROM config
...
config_option.`default`  AS `default`,
config_data.value        AS value,
config_option.mlang      AS mlang,
